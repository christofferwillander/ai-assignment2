...
public class Naive_Bayes {
    ...
    private boolean check_consistent(ArrayList<int[]> arrayList, int condition, int[] query) {
    	...
    	// Each [0][0] represents [1,1] in the game world.
            ...
            for (int x = 1; x <= size; x++) {
                for (int y = 1; y <= size; y++)
                {
                    if(!w.isUnknown(x,y) && w.hasPit(x,y)) {
                    	/* Mark all surrounding squares (they should contain either a stench or a breeze).
                    	 * If 
                    	 * */
        ...
        for (int i = 0; i < conject.size(); i++) {
            ...
            if (conject.get(i)[2] == 1)
            {
                ...
                if(w.isValidPosition(cx - 1, cy)) {
                	marked[cx - 2][cy - 1] = true;
                }
                if(w.isValidPosition(cx, cy + 1)) {
                	marked[cx - 1][cy] = true;
                }
                if(w.isValidPosition(cx, cy - 1)) {
                	marked[cx - 1][cy - 2] = true;
                }
        ...
        for (int x = 1; x <= size; x++) {
            for (int y = 1; y <= size; y++) {
                if (!(w.isUnknown(x, y))) {
                        ...
                        if(condition==PIT) {
                            if (!(w.hasBreeze(x, y) == marked[x-1][y-1])) {
                            	consistant = false;
                            }
                        }

                        else if(condition==WUMPUS) {
                            if (!(w.hasStench(x, y) == marked[x-1][y-1])) {
                            	consistant = false;
                            }
                        }
                }
                ...
                if (!consistant) {
                    return consistant;   /*once the combination is confirmed as inconsistent, break out of for loop*/
                }
            }
